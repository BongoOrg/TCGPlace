...
export class ConversationComponent implements OnInit {
  ...
  salePostId?: string = ''
	...
	conversationSubject: BehaviorSubject<Conversation | null> =
		new BehaviorSubject<Conversation | null>(null) // Utilisation d'un BehaviorSubject pour la conversation
				...
				this.hubConnectionBuilder
					...
					.then(() => {
						this.hubConnectionBuilder.on(
							...
							(idSender: number, idReceiver: number, idMerchPost: string, messageJson: string) => {
	...
	getConversation(): Observable<Conversation> {
		return this.messagesService
			.GetConversation(this.idUser, this.userService.GetCurrentUserID(), this.salePostId)
			.pipe(
				tap((val) => {
					if (val != null) {
