...
export class ConversationListComponent implements OnInit {
	...
	async showConv(conversation: Conversation, idUser: number) {
		await this.router.navigate(['/tabs/messages/conversation'], {
			queryParams: { salePostId: conversation.merchPostId, idUser: idUser }
		})
	}
	...
	handleRefresh(event: any) {
		setTimeout(() => {
			this.isLoading = true
			this.conversations$ = this.messagesService
				.GetAllConversationByUserId(this.currentUserId)
