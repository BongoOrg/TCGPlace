...
export class AddPostPage implements OnDestroy {
	...
	@ViewChild(SearchReferenceComponent)
	searchComponent!: SearchReferenceComponent
	@ViewChild(AddReferencePostComponent)
	addComponent!: AddReferencePostComponent
	...
	ngAfterViewInit() {
		this.subscription.add(
			this.searchComponent.valueSubject
				.pipe(
	...
	searchReferences(value: string): Observable<HttpResponse<PokemonItemReferenceModel[]>> {
		...
		return this.searchService
			.SearchReference(value, this.selectedExtensions, this.currentPage)
			.pipe(
				tap((val) => {
					this.loading = false
					if (val.body !== null) {
