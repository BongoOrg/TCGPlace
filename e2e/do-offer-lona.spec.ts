...
test('do-offer-lona', async ({ page }) => {
  ...
  await page.getByRole('banner').filter({ hasText: 'LonaRègne de GlaceLonaRègne de GlaceLonaRègne de GlaceLonaAstres RadieuxLonaZéni' }).getByRole('img').nth(2).click();
  await page.locator('ion-row').filter({ hasText: /^35 €Lona$/ }).click();
